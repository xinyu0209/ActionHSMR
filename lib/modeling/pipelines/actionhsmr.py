...
from lib.modeling.networks.backbones import TemporalPatchFusion
...
class ActionPipeline(pl.LightningModule):
    ...
    def __init__(self, cfg:DictConfig, name:str, init_backbone=True): 
        ...
        self.cfg = cfg
        self.temporal = TemporalPatchFusion(D=1280, T=3)

         # New timing fusion layer (optional)
        self.temporal_fusion = nn.Sequential(
            nn.Linear(cfg.backbone.embed_dim * 3, cfg.backbone.embed_dim),
            nn.GELU(),
            nn.Linear(cfg.backbone.embed_dim, cfg.backbone.embed_dim)
        ) if cfg.get('use_temporal_fusion', False) else None
    ...
    def configure_optimizers(self):  # There is one optimizer each for the main model and the discriminator
        optimizers = []
    ...
    def _training_step(self, raw_batch, batch_idx):
       ...
       # 3. Prepare the secondary products
        with PM.time_monitor('Secondary Products Preparation'):
            ...
            with PM.time_monitor('Reprojection'):   # 3D key point projection to 2D
    ...
    # 2. backbone
        ...
        feat = self.backbone(x)    # (B*T, D, Hp, Wp)
        _, D, Hp, Wp = feat.shape
        vit_feats = feat.view(B, T, D, Hp, Wp)

        D, Hp, Wp = vit_feats.shape[2:]

    # 3. self-attention
        B, T, D, Hp, Wp = vit_feats.shape
    # (B, T, D, Hp, Wp) 
        temporal_in = vit_feats
        temporal_feat = self.temporal(temporal_in)    # [B, D, Hp, Wp]

    # 3. Run the head to predict the body model parameters.
        with PM.time_monitor('Predict Head Forward'):
            pd_params, pd_cam = self.head(temporal_feat)
   ...
   # 4. Transform the camera parameters to camera translation. 
    ...
    def _adapt_img_inference(self, batch):
        ...
        # Process the frame rate and complete it to 3 frames
