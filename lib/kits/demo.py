...
from lib.modeling.pipelines.actionhsmr import build_inference_pipeline
...
def parse_args():
    ...
    parser.add_argument('-d', '--device', type=str, default='cuda:1', help='The device.')
...
def visualize_full_img(pd_cam_t, raw_imgs, det_meta, m_skin, m_skel, have_caption:bool=False) -> Tuple[List[np.ndarray], np.ndarray]:
    ...
    for i in tqdm(range(len(raw_imgs)), desc='Rendering'):
        ...
        full_img_bg = np.ones((1024, 1024, 3), dtype=np.uint8) * 255
        render_results = {}
